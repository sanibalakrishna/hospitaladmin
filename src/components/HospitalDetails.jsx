...
function HospitalDetails({ edit, setEdit }) {
  ...
  const [address, setAddress] = useState(edit.data.address);
  const [updateedit, setUpdateedit] = useState(false);
  const updateDoctorDetails = async () => {
    const response = await fetch(
      `https://hospitalapi.vercel.app/hospitals/${edit.data._id}`,
      {
        method: "PATCH",
  ...
  const handleDelete = async () => {
    ...
    const json = await response.json();
    if (response.ok) {
      toast.success("Deleted Hospital from DB Successfully");
